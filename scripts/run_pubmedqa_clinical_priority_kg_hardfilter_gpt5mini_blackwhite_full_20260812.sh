#!/usr/bin/env bash
# PubMedQA complete black-box/white-box comparison with GPT-5 mini and the new BIOS KG.
set -euo pipefail
...
CACHE=results/medical_kg_caches/mirage_pubmedqa_all_gpt5_mini_contriever_triplets.jsonl
QUERY_DIR=formal_dot_pubmedqa_clinical_priority_kg_original_hardfilter_gpt5_mini_blackwhite_full_20260812
LOG_ROOT=logs/formal_dot_pubmedqa_clinical_priority_kg_original_hardfilter_gpt5_mini_blackwhite_full_20260812
RUN_GPU=2
MIN_AVAILABLE_KIB=$((220 * 1024 * 1024))

mkdir -p "$LOG_ROOT" "$(dirname "$CACHE")"
touch "$CACHE"
printf '%s\n' "$$" > "$LOG_ROOT/queue.pid"
...
cache_path = Path("results/medical_kg_caches/mirage_pubmedqa_all_gpt5_mini_contriever_triplets.jsonl")

for path in (retrieval_path, metadata_path, source_path, ids_path, config_path, cache_path):
...
print("input validation passed: PubMedQA=500, gpt-5-mini, Contriever/raw-dot, new clinical-priority KG, original hard-filter=1:0, no TrustRAG")
PY
}
...
run_experiment() {
  local attack=$1
  local variant=$2
  local name="formal_dot_pubmedqa_contriever_clinical_priority_kg_original_hardfilter_gpt5_mini_${variant}_full_20260812"
  ...
  wait_for_memory
  echo "$(date '+%F %T') starting PubMedQA GPT-5 mini clinical-priority KG hard-filter/${variant} on GPU${RUN_GPU}"
  CUDA_VISIBLE_DEVICES="$RUN_GPU" "$PYTHON" -u main.py \
    ...
    --medical_kg_triplet_cache_path "$CACHE" \
    --medical_kg_triplet_cache_namespace triplet_schema_v1_gpt5_mini \
    --name "$name" > "$log" 2>&1
  is_complete_output "$output" "$attack"
  echo "$(date '+%F %T') completed PubMedQA GPT-5 mini clinical-priority KG hard-filter/${variant}"
}
