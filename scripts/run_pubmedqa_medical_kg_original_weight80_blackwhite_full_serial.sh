#!/usr/bin/env bash
# Full PubMedQA KG-only comparison at safety:retrieval = 8:2.
# Runs black-box LM_targeted and white-box HotFlip serially so their shared
# document-hash triplet cache is never written concurrently.
set -euo pipefail
...
QUERY_DIR=formal_dot_pubmedqa_medical_kg_original_weight80_blackwhite_full_20260806
LOG_ROOT=logs/formal_dot_pubmedqa_medical_kg_original_weight80_blackwhite_full_20260806
...
print("input validation passed: PubMedQA=500, Contriever/raw-dot, own-5, KG safety:retrieval=8:2, shared triplet cache present")
PY
}
...
assert all(abs(float(row.get("medical_kg_rerank_weight", -1.0)) - 0.80) < 1e-9 for row in rows)
...
run_experiment() {
  local attack=$1
  local variant=$2
  local name="formal_dot_pubmedqa_contriever_medical_kg_original_weight80_${variant}_full_20260806"
  ...
  wait_for_memory
  echo "$(date '+%F %T') starting PubMedQA KG-only/${variant} on GPU4, safety:retrieval=8:2"
  ...
  echo "$(date '+%F %T') completed PubMedQA KG-only/${variant}, safety:retrieval=8:2"
}
