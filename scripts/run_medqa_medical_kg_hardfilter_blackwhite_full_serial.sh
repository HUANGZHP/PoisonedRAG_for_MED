#!/usr/bin/env bash
# MedQA 1:0 safety decision: remove identified malicious passages directly.
set -euo pipefail
...
QUERY_DIR=formal_dot_medqa_medical_kg_original_hardfilter_blackwhite_full_20260810
LOG_ROOT=logs/formal_dot_medqa_medical_kg_original_hardfilter_blackwhite_full_20260810
...
run_experiment() {
  local attack=$1
  local variant=$2
  local name="formal_dot_medqa_contriever_medical_kg_original_hardfilter_${variant}_full_20260810"
  ...
  wait_for_memory
  echo "$(date '+%F %T') starting MedQA binary KG hard-filter/${variant} on GPU7"
