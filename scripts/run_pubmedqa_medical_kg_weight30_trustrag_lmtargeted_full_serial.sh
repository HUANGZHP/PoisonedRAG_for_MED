#!/usr/bin/env bash
# Full PubMedQA PoisonedRAG comparison: medical KG + original TrustRAG at
# safety:retrieval = 3:7.  It uses the existing black-box LM_targeted protocol.
set -euo pipefail
...
# This pre-existing cache is document-hash keyed and contains no query or label data.
SHARED_CACHE=results/medical_kg_caches/mirage_pubmedqa_all_gpt41mini_contriever_triplets.jsonl
QUERY_DIR=formal_dot_pubmedqa_medical_kg_original_weight30_trustrag_lmtargeted_full_20260810
LOG_ROOT=logs/formal_dot_pubmedqa_medical_kg_original_weight30_trustrag_lmtargeted_full_20260810
...
print("input validation passed: PubMedQA=500, Contriever/raw-dot, own-5, KG safety:retrieval=3:7 + original TrustRAG, shared triplet cache present")
PY
}
...
assert all(abs(float(row.get("medical_kg_rerank_weight", -1.0)) - 0.30) < 1e-9 for row in rows)
assert all(row.get("trustrag_filter_enabled") is True for row in rows)
assert attack == "LM_targeted"
PY
}
...
run_experiment() {
  local attack=$1
  local variant=$2
  local name="formal_dot_pubmedqa_contriever_medical_kg_original_weight30_trustrag_${variant}_full_20260810"
  ...
  wait_for_memory
  echo "$(date '+%F %T') starting PubMedQA KG+original-TrustRAG/${variant} on GPU4, safety:retrieval=3:7"
  CUDA_VISIBLE_DEVICES=4 "$PYTHON" -u main.py \
    ...
    --medical_kg_rerank_weight 0.30 --trustrag_filter \
  ...
  echo "$(date '+%F %T') completed PubMedQA KG+original-TrustRAG/${variant}, safety:retrieval=3:7"
}
