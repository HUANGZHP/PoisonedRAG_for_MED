...
# 对单一评测集顺序运行 Contriever v4 的无攻击、Black-box、HotFlip 无防御实验。
set -u -o pipefail
...
run() {
    local suffix="$1"
    local attack="$2"
    local name="${LABEL}_contriever_v4_gpt41mini_no_defense_${suffix}"
    printf '%s starting %s\n' "$(date -Is)" "$name" >> "$ORCHESTRATOR"
    "$PYTHON" -u main.py \
        --eval_model_code contriever_v4 --eval_dataset pubmed --model_name gpt4.1mini \
