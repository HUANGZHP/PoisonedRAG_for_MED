#!/usr/bin/env bash
# Run the corrected original-Contriever MedQA TrustRAG matrix using the
...
LOG_ROOT=logs/formal_dot_contriever_medqa_defenses_corrected
PID_FILE="$LOG_ROOT/trustrag_medqa_corrected_attacks_serial.pid"
...
run_eval() {
  local defense=$1
  local attack=$2
  local query_dir="formal_dot_contriever_medqa_${defense}_corrected"
  local name="formal_dot_medqa_contriever_${defense}_corrected_${attack}"
...
echo "$(date '+%F %T') corrected MedQA TrustRAG attack queue complete"
