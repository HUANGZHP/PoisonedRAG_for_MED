#!/usr/bin/env bash
# Full PubMedQA medical-KG evaluation queue.  It waits for the currently
# running MedQA white-box queue, then runs KG-only and KG+original-TrustRAG
# serially without rebuilding the validated raw-dot retrieval file.
set -euo pipefail
...
LOG_ROOT=logs/formal_dot_pubmedqa_medical_kg_original_full_20260803
OLD_QUEUE_PID_FILE=logs/formal_dot_original_contriever_pubmedqa_clean_medqa_trustrag_n60_20260803/queue.pid
...
print("input validation passed: PubMedQA=500, Contriever/raw-dot, own-5, medical-KG artifact complete")
PY
}

wait_for_previous_queue() {
  if test ! -s "$OLD_QUEUE_PID_FILE"; then
    return
  fi
  local old_pid
  old_pid=$(tr -d '[:space:]' < "$OLD_QUEUE_PID_FILE")
  while [[ "$old_pid" =~ ^[0-9]+$ ]] && kill -0 "$old_pid" 2>/dev/null; do
    echo "$(date '+%F %T') waiting: prior MedQA n=60 queue pid=${old_pid} is still active"
    sleep 60
  done
}
...
assert all(str(row.get("parsed_pred_label", "")).strip().lower() in {"yes", "no", "maybe"} for row in rows)
...
run_experiment() {
  ...
  local query_dir=formal_dot_pubmedqa_medical_kg_original_full_20260803
  local name="formal_dot_pubmedqa_contriever_medical_kg_original_full_20260803_${variant}"
  ...
  wait_for_memory
  echo "$(date '+%F %T') starting PubMedQA full medical-KG/${variant} on GPU4"
  local extra_args=()
  ...
  echo "$(date '+%F %T') completed PubMedQA full medical-KG/${variant}"
}
