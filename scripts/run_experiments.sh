...
# 先生成 Contriever v3 的两份 PubMed 检索结果，再并行启动与 v2 相同的无防御评测。
set -u -o pipefail
...
printf '%s v3 retrieval and experiment pipeline started\n' "$(date -Is)" > "$ORCHESTRATOR"
...
run_retrieval() {
    ...
    "$PYTHON" -u evaluate_beir.py \
        --model_code contriever_v3 --dataset pubmed --top_k 100 --gpu_id 3 \
...
nohup bash scripts/run_contriever_v3_no_defense_suite.sh \
    ...
    results/beir_results/mirage_medqa_all-contriever_v3.json 6 \
    > "$LOG_DIR/medqa_pubmed_no_defense_pipeline.log" 2>&1 < /dev/null &

printf '%s both v3 suites launched\n' "$(date -Is)" >> "$ORCHESTRATOR"
